...
class DevServerUploadToS3Plugin {
    constructor({ options = {}, params = {} }) {
        const region = (options.region && options.region.length > 0) ? "-" + options.region : "";
        this.publicUrl = `https://${params.Bucket}.s3${region}.amazonaws.com/${params.Key}/index.html`;
        this.params = params;
    ...
    apply(compiler) {
        ...
        compiler.hooks.done.tap(PLUGIN_NAME, stats => {
            ...
            console.debug("hotReloadFiles: " + hotReloadFiles);
            if (hotReloadFiles.length > 0) {
                // this.lastUploadedHotReloadFiles.push([...hotReloadFiles]);
                this.lastUploadedHotReloadFiles = hotReloadFiles;
            }
    ...
    deletePreviousHotReloadFiles() {
        console.debug("deleting obsolete hot-update files ... ");
        ...
        return new Promise((resolve, reject) => {
            ...
            let filesToDelete = [];
            const lastUploaded = this.flat(this.lastUploadedHotReloadFiles);
